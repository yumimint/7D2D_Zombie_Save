...
def load_i18n_strings():
    ...
    if lang in i18n:
        STRINGS.update(i18n[lang])
        for key, text in i18n["en_US"].items():
            if key not in STRINGS:
                STRINGS[key] = text
    else:
        STRINGS.update(i18n["en_US"])
...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        # 各列にラベルを追加（仮のウィジェット）
        tk.Label(self.column1, text=STRINGS["save_data_list_label"]).pack()
        ...
        ttk.Button(
            ...
            style="TButton",
        ).pack(side=tk.TOP, padx=5, pady=5, fill=tk.X)
    ...
    def monitor_loop(self):
        ...
        self.monitored_mtimes = {
            savedir: get_mtime(savedir) for savedir in iter_savedir()
        }
    ...
    def load_save_data(self):
        ...
        # セーブデータの一覧を取得
        self.saves = sorted(
            iter_savedir(), key=lambda x: cashed_mtime_of_tree(x), reverse=True
        )  # 最新順に
    ...
    def load_backup_data(self, selected_save: Path = None):
        ...
        self.restore_button.config(
            state=tk.DISABLED
        )  # バックアップリスト更新時は復元ボタンを無効化

        if selected_save is None:
            self.backups = list(BACKUPS.glob("*.zip"))
        else:
            # selected_saveが在れば関連するzipだけリスト化
            self.backups = list(BACKUPS.glob(f"*{selected_save.name}_*.zip"))

        # 更新日時でソート
        self.backups = sorted(
            self.backups, key=lambda x: x.stat().st_mtime, reverse=True
        )
    ...
    def do_backup(self):
        ...
        if not selected_indices:
            showerror(
                STRINGS["error_title"], STRINGS["select_save_for_backup_error_msg"]
            )
            return
    ...
    def do_restore(self):
        ...
        if not selected_backup_indices:
            showerror(
                STRINGS["error_title"], STRINGS["select_backup_for_restore_error_msg"]
            )
            return
        ...
        if save_dir.exists():
            confirm_message = STRINGS["restore_confirm_msg"].format(
                backup_filename=backup_file.name, savename=savename
            )
...
def create_backup(savedir: Path, *, if_exists="confirm"):
    ...
    shutil.make_archive(
        str(archive_name_base),
        "zip",
        SAVES,
        f"{savedir.parent.name}/{savedir.name}",
        logger=logger,
    )
